...
from resume.forms import LangForm as SkillForm
...
def skills(request):
    skills = models.Language.objects.filter(resume__user=request.user)
    id = models.Resume.objects.get(user=request.user).id
    context = {
        'skills': skills,
        'id':id,
    }
    return render(request, 'resume/lang/skills.html', context)
...
def add_skill(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                skills = models.Language.objects.create(name=form.cleaned_data['name'], level=form.cleaned_data['level'], resume=resume)

                skills.save()
                messages.success(request, f" زبان  {form.cleaned_data['name']} با موفقیت اضافه شد. ")
                return HttpResponseRedirect(reverse_lazy("resume_lang"))
            except:
    ...
    return render(request, 'resume/lang/skills_add.html', context)

def delete_skill(request, id):
    try:
        skill = models.Language.objects.get(id=id, resume__user=request.user)
